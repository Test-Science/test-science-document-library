...
num_posts <- nrow(dat)
print(c("Number of posts:", num_posts))
...
for (i in 1:num_posts) {
  ...
  slug <- gsub(
    "/index",
    "",
    blogdown:::trim_ws(blogdown:::post_filename(
      dat[i, ]$Project,
      "",
      "",
      dat[i, ]$Year
...
paste0(
  "[",
  paste(
    paste0("'", strsplit(dat[i, ]$Area, split = ", ")[[1]], "'"),
    collapse = ","
  ),
  "]"
),
...
paste0(
  "[",
  paste(
    paste0("'", strsplit(dat[i, ]$Keywords, split = ", ")[[1]], "'"),
    collapse = ","
  ),
  "]"
),
...
paste0(
  "[",
  paste(
    paste0("'", strsplit(dat[i, ]$Authors, split = ", ")[[1]], "'"),
    collapse = ","
  ),
  "]"
),
...
ifelse(
  ...
  paste(
    ...
    regmatches(
      dat[i, ]$Video,
      regexpr("(?<=watch\\?v=).*$", dat[i, ]$Video, perl = TRUE)
    ),
    ">}}"
  ),
  ""
...
# Abstract, if present
ifelse(!is.na(dat[i, ]$Description), paste(dat[i, ]$Description), ""),
